"""
Local MemoryStore — JSON-backed key-value store, replaces memory-store package.
"""
...
class MemoryStore:
    ...
    def _search_dict(
        self, data: dict, path: list[str], needle: str,
        results: list[dict[str, Any]],
    ):
        for k, v in data.items():
            ...
            if any(needle in hay for hay in haystacks):
                ...
                results.append({"category": category, "key": k, "value": v})
    ...
    def _entry_matches(
        self, needle: str, category: str,
        item_key: str, item_value: Any,
    ) -> bool:
        haystacks = [
