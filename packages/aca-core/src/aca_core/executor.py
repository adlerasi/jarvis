...
Standalone library version: tool dispatch and shell execution are injected
via callables / handler dicts.
"""
...
from aca_core.task_graph import ActionType, TaskNode


class Executor:
    """Executes ACA task steps by dispatching to tools, pyautogui, or shell.

    ``tool_handlers`` — optional dict ``{tool_name: callable(name, params) → str}``
    ``shell_executor`` — optional callable ``(command: str) → str``
    ``pyautogui_executor`` — optional callable ``(params: dict) → str``

    Each execute_step() has a default 30-second timeout enforced via
    ThreadPoolExecutor.
    """
    ...
    def execute_step(self, node: TaskNode, _world_state: dict[str, Any]) -> dict[str, Any]:
        result: dict[str, Any] = {
            "success": False,
            "result": "",
            "error": None,
    ...
    def _execute_shell(self, params: dict[str, Any]) -> str:
        ...
        return "aca-core: shell executor not configured."
