"""Agent Memory — JSON-backed persistence for goals, sessions, and templates."""
...
class AgentMemory:
    ...
    Storage paths are configurable via ``base_path``.  All data is stored as
    JSON files on disk and survives application restart.
    """
    ...
    def save_session(self, session_id: str, session_data: dict[str, Any]) -> bool:
        try:
            ...
            index["sessions"][session_id] = {
                "goal_id": session_data.get("goal", {}).get("goal_id", ""),
                "status": session_data.get("goal", {}).get("status", ""),
                "updated_at": time.time(),
            }
            self._write_index(index)
            return True
    ...
    def list_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        result = []
        for sid, sdata in sorted_sessions[:limit]:
            result.append({"session_id": sid, **sdata})
        return result
    ...
    def find_templates(self, intent_text: str) -> list[dict[str, Any]]:
        normalized = self._normalize_text(intent_text)
    ...
    def _normalize_text(text: str) -> str:
        ...
        stopwords = {
            "ve", "bir", "bu", "su", "o", "ile", "icin", "olarak",
            "olan", "gereken", "daha", "en", "cok", "az",
    ...
    def compute_similarity(a: str, b: str) -> float:
        norm_a = AgentMemory._normalize_text(a)
        norm_b = AgentMemory._normalize_text(b)
