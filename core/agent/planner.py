"""Planner — LLM-based goal decomposition into executable task graphs."""
...
from core.agent.task_graph import ActionType, TaskGraph, TaskNode
...
class Planner:
    ...
    ``llm_callable`` — optional ``(prompt: str) -> str | None`` that sends a
    planning prompt to an LLM and returns a JSON string.  When omitted the
    planner falls back to ``_fallback_plan``.
    """
    ...
    def replan(
        ...
        if self._llm_callable is not None:
            try:
                ...
                llm_result = self._llm_callable(full_prompt)
                if llm_result:
                    graph = self._parse_llm_response(llm_result, previous_graph.goal_id)
                    if self._validate_graph(graph):
                        return graph
            except Exception:
                traceback.print_exc()
        ...
        g = TaskGraph(
            graph_id=f"replan_{int(time.time())}",
        ...
        try:
            ...
            for match in matches:
                ...
                if score >= 0.7:
                    ...
                    for step_data in match.get("steps", []):
                        node = TaskNode(
                            step_id=step_data["step_id"],
                            description=step_data["description"],
                            action_type=step_data.get("action_type", "tool"),
    ...
    def _fallback_plan(self, goal_text: str) -> TaskGraph:
        g = TaskGraph(
            graph_id=f"fallback_{int(time.time())}",
            goal_id=goal_text,
        )
        g.add_node(TaskNode(
            step_id="s1",
